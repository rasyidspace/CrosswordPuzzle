...
export const Page11Material5: React.FC = () => {
  const mat = MATERIALS[5];
              ...
              {sub.examples && (
                <div className="flex flex-wrap gap-1.5 mt-1 pt-1.5 border-t border-amber-100">
                  {sub.examples.map((ex, exIdx) => (
                    <span
                      key={exIdx}
                      className="px-2.5 py-0.5 rounded-lg bg-amber-50 text-[11px] font-extrabold text-amber-800 border border-amber-200"
                    >
                      ✨ {ex}
                    </span>
                  ))}
